...
const STORAGE_KEY = "identificationScore";

const Identification = () => {
  const identificationQuiz = quiz.filter(
    (q) => q.category === "Identification",
  );
  ...
  const handleChoice = (choice) => {
    ...
    const questionId = identificationQuiz[currentQuestion].id;
    ...
    const isCorrect =
      choice === identificationQuiz[currentQuestion].correctAnswer;
  ...
  const handleNext = () => {
    ...
    if (currentQuestion < identificationQuiz.length - 1) {
      setCurrentQuestion((prev) => prev + 1);
    }
  };

  const question = identificationQuiz[currentQuestion];
      ...
      >
        ...
        <View className="mb-4">
          <Text className="text-2xl font-bold text-purple-900">
            Dressmaking Identification Challenge
          </Text>
          <Text className="text-purple-700 mt-2">
            Identify the correct body measurement based on the description.
            Choose wisely—each correct answer adds to your score.
          </Text>
        </View>
        ...
        <View className="mb-4 bg-purple-200 p-4 rounded-xl">
          <Text className="text-purple-900 font-bold">
            Score: {score} / {identificationQuiz.length}
          </Text>
          <Text className="text-purple-800 text-sm">
            Question {currentQuestion + 1} of {identificationQuiz.length}
          </Text>
        </View>
        ...
        <View className="bg-purple-200 rounded-2xl p-6 shadow-lg">
          ...
          {showFeedback && (
            <View className="mt-4 p-4 bg-purple-100 rounded-xl shadow">
              ...
              {currentQuestion < identificationQuiz.length - 1 && (
                <Pressable
                  onPress={handleNext}
